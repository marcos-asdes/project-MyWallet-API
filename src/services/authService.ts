...
async function checkIfUserIsAlreadyRegistered(email: string): Promise<void> {
  const user: WithId<Document> | null = await authRepository.findEmailInDatabase(email);
  if (user && user.length !== 0) throw new ErrorLog(409, 'User already exists');
...
async function checkIfUserIsRegistered(email: string): Promise<SignInType> {
  const user = await authRepository.findEmailInDatabase(email);
...
function generateToken(userId: ObjectId) {
  const data = {};
  const subject: string = String(userId);
